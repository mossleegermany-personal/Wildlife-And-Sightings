...
function buildInfoPanelSvg(data) {
  const commonName = truncate(data.commonName || data.common_name || 'Unknown', 28);
  ...
  const country = truncate(data.country || '', 30);
  ...
  for (const badge of badges.slice(0, 3)) {
    ...
    badgesSvg += `
      ...
      <text x="${bx + bw / 2}" y="${by + 16}" font-size="12" fill="#ddd" text-anchor="middle" font-family="sans-serif">${badge}</text>`;
    bx += bw + 8;
  }
  ...
  if (accuracy) {
    ...
    accuracySvg = `
      <text x="20" y="298" font-size="12" fill="#888" font-family="sans-serif">Confidence</text>
      <rect x="20" y="308" width="${PANEL_WIDTH - 40}" height="8" rx="4" fill="#333" />
      <rect x="20" y="308" width="${barW}" height="8" rx="4" fill="${iucnColor}" />
      <text x="${PANEL_WIDTH - 20}" y="318" font-size="12" fill="#aaa" text-anchor="end" font-family="sans-serif">${Math.round(pct)}%</text>`;
  }
  ...
  return `
    ...
    <!-- Common name -->
    <text x="20" y="50" font-size="22" fill="#f0f0f0" font-weight="bold" font-family="sans-serif">${escSvg(commonName)}</text>
    <!-- Scientific name -->
    <text x="20" y="76" font-size="14" fill="#aaa" font-style="italic" font-family="sans-serif">${escSvg(sciName)}</text>
    <!-- Divider -->
    ...
    <text x="32" y="125" font-size="12" fill="${iucnColor}" font-weight="bold" font-family="sans-serif">${escSvg(iucnStatus)}</text>
    <!-- Country -->
    ${country ? `<text x="20" y="162" font-size="13" fill="#bbb" font-family="sans-serif">📍 ${escSvg(country)}</text>` : ''}
    <!-- Accuracy bar -->
    ${accuracySvg}
    <!-- Badges -->
    ${badgesSvg}
    <!-- Footer -->
    <text x="${PANEL_WIDTH / 2}" y="${IMAGE_HEIGHT - 12}" font-size="11" fill="#555" text-anchor="middle" font-family="sans-serif">Wildlife Sightings API</text>
  `;
}
...
function buildResultPanel(data, W, H) {
  ...
  if (!_skip(data.localStatus)) {
    headerBadges.push({ label: cleanBadgeLabel(data.localStatus), color: '#38bdf8' });
  }
  ...
  if (!_skip(sexVal) && sexVal !== 'Unknown') {
    ...
    if (sexVal.toLowerCase() === 'male') sexLabel += ' ♂';
    else if (sexVal.toLowerCase() === 'female') sexLabel += ' ♀';
  ...
  if (headerBadges.length > 0) {
    ...
    headerBadges.forEach((badge, idx) => {
      ...
      headerBadgesSvg += `
        ...
        <text x="${bx + badgeW / 2}" y="${by + 17}" font-size="12" fill="${badge.color}" text-anchor="middle" font-family="sans-serif" font-weight="600">${escSvg(text)}</text>`;
    });
  }
  ...
  const commonLines = wordWrap(data.commonName || 'Unknown', 26).slice(0, 2);
  const sciLines    = wordWrap(data.scientificName || '', 36).slice(0, 2);
  ...
  commonLines.forEach(line => {
    nameSvg += `<text x="16" y="${curY}" font-size="30" fill="#f8fafc" font-weight="bold" font-family="sans-serif">${escSvg(line)}</text>`;
    curY += 38;
  });
  curY += 6;
  sciLines.forEach(line => {
    nameSvg += `<text x="16" y="${curY}" font-size="18" fill="#7dd3fc" font-style="italic" font-family="sans-serif">${escSvg(line)}</text>`;
    curY += 26;
  });
  ...
  if (data.subspecies && Array.isArray(data.subspecies) && data.subspecies.length > 0 && (data.subspeciesFromImage || data.subspeciesByLocation)) {
    ...
    nameSvg += `<text x="16" y="${curY}" font-size="13" fill="#94a3b8" font-family="sans-serif">${subspLabel}</text>`;
    curY += 20;
    for (const grp of data.subspecies.slice(0, 6)) {
      ...
      nameSvg += `<text x="24" y="${curY}" font-size="13" fill="#86efac" font-style="italic" font-family="sans-serif">• ${escSvg(epithet)}</text>`;
      curY += 18;
    }
  }
  ...
  const loc = data.ebirdSightingsLocation || 'Singapore';
  if (typeof data.ebirdSightingsCount === 'number' && data.ebirdSightingsCount >= 0) {
    ebirdSightingsFooter = `<text x="${W / 2}" y="${H - 42}" font-size="32" fill="#facc15" text-anchor="middle" font-family="sans-serif" font-weight="bold">No. of Sightings (${escSvg(loc)}): ${data.ebirdSightingsCount}</text>`;
  } else {
    ebirdSightingsFooter = `<text x="${W / 2}" y="${H - 42}" font-size="32" fill="#facc15" text-anchor="middle" font-family="sans-serif" font-weight="bold">No. of Sightings (${escSvg(loc)}): N/A</text>`;
  }
